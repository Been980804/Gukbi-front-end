  ...
  <div class="container">
    ...
    <div class="main_container">
      ...
      <div class="m_show_box">
        <div class="m_show_box_title">도서예약 전체 목록</div>
        ...
        <div class="table_line_box">
          <table class="m_table">
            ...
            <tbody>
              <tr v-for="reserv in reservList" :key="reserv">
                <td class="m_td">{{ reserv.book_no }}</td>
                <td class="m_td">{{ reserv.book_title }}</td>
                <td class="m_td">{{ reserv.mem_no }}</td>
                <td class="m_td">{{ reserv.mem_name }}</td>
                <td class="m_td">{{ reserv.reserv_date }}</td>
                <td class="m_td text_green" v-if="reserv.reserv_state == `예약완료`">{{ reserv.reserv_state }}</td>
                <td class="m_td text_red" v-else>{{ reserv.reserv_state }}</td>
              </tr>
            </tbody>
          </table>
        </div>
...
export default {
  ...
  data() {
    return {
      selectOptions: ["도서명", "회원명"],
      selectOptionsColumn: ["book.book_title", "member.mem_name"],
      selectedOption: "도서명",
      isOpen: false,
      searchText: "", // 검색했을때 검색 내용 저장
      searchOption: "", // 검색했을때 검색 컬럼 저장

      columnRange: ["10%", "30%", "15%", "15%", "15%", "15%"],
      tableColumn: ["도서코드", "도서명", "회원코드", "회원명", "예약일", "예약상태"],
  ...
  methods: {
    ...
    setSearchText(event) { // 검색 내용 저장
      ...
      this.getReservCount(); // 검색 내용 저장 후 새로 조회
    },
    ...
    getReservCount() {
      ...
      api.get(`/manage/reserv/reservCount`, {params: Object.fromEntries(sqlData)})
        .then(res => {
          if (res.common.res_code == 200) { // 응답성공
            this.totalCount = res.data.reservCount;
            
            if (this.totalCount > 0) {
              // 테이블 데이터가 있을 경우 조회
              this.getReservInfo(this.currentPage);
              this.getViewPage();
            }
          } else { // 응답실패
    ...
    getReservInfo() {
      ...
      api.get(`/manage/reserv/reservList/${this.currentPage}`, {params: Object.fromEntries(sqlData)})
        .then(res => {
          if (res.common.res_code == 200) { // 응답성공
            this.reservList = res.data.reservList;
          } else { // 응답실패
            console.log("RentalListView rental/rentalList 응답실패");
          }
        })
    },
