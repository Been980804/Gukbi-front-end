  ...
  <div class="container">
    ...
    <div class="main_container">
      ...
      <div class="m_show_box">
        <div class="m_show_box_title">회원 전체 목록</div>
...
export default {
  ...
  data() {
    return {
      selectOptions: ["회원명", "아이디"],
      selectOptionsColumn: ["mem_name", "mem_id"],
      selectedOption: "회원명",
      isOpen: false,
      searchText: "", // 검색했을때 검색 내용 저장
      searchOption: "", // 검색했을때 검색 컬럼 저장

      columnRange: ["10%", "10%", "10%", "15%", "15%", "15%", "10%", "15%"],
      tableColumn: ["회원코드", "회원명", "아이디", "전화번호", "생년월일", "가입일", "대여가능여부", "대여내역확인"],
  ...
  methods: {
    ...
    setSearchText(event) { // 검색 내용 저장
      ...
      this.getUserCount(); // 검색 내용 저장 후 새로 조회
    },
    ...
    getUserCount() {
      ...
      api.get(`/manage/user/userCount`, {params: Object.fromEntries(sqlData)})
        .then(res => {
          if (res.common.res_code == 200) { // 응답성공
            this.totalCount = res.data.userCount;
            ...
            console.log("UserListView user/userCount 응답실패");
          }
        })
    },

    getUserInfo() { // 현재 페이지의 책 리스트 정보 가져오기
      let sqlData = new Map();
      sqlData.set("column", this.searchOption);
      sqlData.set("search", this.searchText);

      api.get(`/manage/user/userList/${this.currentPage}`, {params: Object.fromEntries(sqlData)})
        .then(res => {
          if (res.common.res_code == 200) { // 응답성공
            this.userList = res.data.userList;
          } else { // 응답실패
            console.log("UserListView user/userList 응답실패");
          }
        })
    },
