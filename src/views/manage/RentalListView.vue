          ...
          <table class="m_table">
            ...
            <tbody>
              <tr v-for="rent in rentList" :key="rent">
                ...
                <td class="m_td">{{ rent.mem_name }}</td>
                ...
                <td class="m_td text_blue" v-if="rent.return_state == `대여중`">{{ rent.return_state }}</td>
                <td class="m_td text_red" v-else-if="rent.return_state == `연체중`">{{ rent.return_state }}</td>
                <td class="m_td text_red" v-else-if="rent.return_state == `연체 후 반납`">{{ rent.return_state }}</td>
                <td class="m_td text_green" v-else>{{ rent.return_state }}</td>
              </tr>
            </tbody>
          </table>
        </div>
...
export default {
  ...
  methods: {
    ...
    getRentCount() {
      ...
      api.get(`/manage/rental/rentCount`, {params: Object.fromEntries(sqlData)})
        .then(res => {
          if (res.common.res_code == 200) { // 응답성공
            ...
            if (this.totalCount > 0) {
              // 테이블 데이터가 있을 경우 조회
              this.getRentalInfo(this.currentPage);
              this.getViewPage();
            }
          } else { // 응답실패
    ...
    getRentalInfo() {
      ...
      api.get(`/manage/rental/rentalList/${this.currentPage}`, {params: Object.fromEntries(sqlData)})
        .then(res => {
          if (res.common.res_code == 200) { // 응답성공
            this.rentList = res.data.rentList;
          } else { // 응답실패
