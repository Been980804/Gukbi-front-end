  ...
  <div class="container">
    ...
    <div class="main_container">
      ...
      <div class="m_show_box">
        <div class="m_show_box_title">희망도서 전체 목록</div>
...
export default {
  ...
  data() {
    return {
      selectOptions: ["제목", "신청자"],
      selectOptionsColumn: ["appl.appl_title", "member.mem_name"],
      selectedOption: "제목",
      isOpen: false,
      searchText: "", // 검색했을때 검색 내용 저장
      searchOption: "", // 검색했을때 검색 컬럼 저장

      columnRange: ["10%", "40%", "10%", "10%", "15%", "15%"],
      tableColumn: ["신청자", "도서명", "저자", "출판사", "신청일", "신청상태"],
  ...
  methods: {
    ...
    setSearchText(event) { // 검색 내용 저장
      ...
      this.getApplCount(); // 검색 내용 저장 후 새로 조회
    },
    ...
    getApplCount() {
      ...
      api.get(`/manage/board/appl/applCount`, {params: Object.fromEntries(sqlData)})
        .then(res => {
          if (res.common.res_code == 200) { // 응답성공
            this.totalCount = res.data.applCount;

            if (this.totalCount > 0) {
              // 테이블 데이터가 있을 경우 조회
              this.getApplInfo(this.currentPage);
              this.getViewPage();
            }
          } else { // 응답실패
    ...
    getApplInfo() {
      ...
      api.get(`/manage/board/appl/applList/${this.currentPage}`, {params: Object.fromEntries(sqlData)})
        .then(res => {
          if (res.common.res_code == 200) { // 응답성공
            this.applList = res.data.applList;
          } else { // 응답실패
